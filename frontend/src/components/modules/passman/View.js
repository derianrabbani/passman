...
export default function Index() {
    ...
    const getData = async () => {
        ...
        const view = await Axios.get("http://localhost:5000/passman/", { headers: { "auth-token": token } })
    ...
    const deleted = async (id) => {
        ...
        try {
            if (window.confirm("Are you sure delete this document ?")) {
                ...
                const deleted = await Axios.delete("http://localhost:5000/passman/delete/" + id, { headers: { "auth-token": token } })
    ...
    const showEncryption = async (id) => {
        try {
            ...
            const showPassword = await Axios.get("http://localhost:5000/passman/" + id)
